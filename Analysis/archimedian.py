...
class ArchimedianProperty(Scene):
    ...
    def prove_it(self):
        anti_hyothesis = (
            MathTex(r"\forall n\in\mathbb{N},\ nx \le y").shift(UP).to_edge(LEFT)
        )
        a_major = (
            MathTex(
                r"\Rightarrow",
                r"A=\{nx|n\in\mathbb{N}\} \text{ et majoré}",
                r", s=\sup{A}",
            )
            .next_to(anti_hyothesis, DOWN)
            .to_edge(LEFT)
        )
        caracterise = (
            MathTex("s-x < nx", r"\Rightarrow s < (n+1)x", r"\in A", r"\ \square")
            .next_to(a_major, DOWN)
            .to_edge(LEFT)
        )
    ...
    def illustrate_property(self):
        ...
        self.play(Create(x_segment), Create(y_segment), Write(x_label), Write(y_label))
